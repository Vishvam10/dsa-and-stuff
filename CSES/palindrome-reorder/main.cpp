...
void solve()
{
  ...
  // For push_front. We could use pre-filled strings (string ans(n, '')) as well
  deque<char> qu;
  ...
  // Start with odd valued key first so that we can get them in middle like 
  // this : . . . . x x x . . . . 
  if(flag) {
    int rep = mp.find(odd_val_key)->second;
    for(int i = 0; i < rep; i++) {
      qu.push_back(odd_val_key);
    }
  }
  ...
  for(const auto&[k, v] : mp) {
    ...
    // Here, v is guaranteed to be even
    for(int i = 0; i < v / 2; ++i) {
      qu.push_back(k);
    }
