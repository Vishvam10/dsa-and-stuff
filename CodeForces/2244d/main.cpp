...
int sum(int n, vector<int>& prod, pair<int, int> p) {
    if(p.first == 0 && p.second == 0) return INT_MIN;
    int low = min(n, p.first), high = max(n, p.second), ans = 0;
    for(int i = low + 1; i < high; ++i) ans += (prod[i] * -1);
    cout << "ans : " <<  ans << "\n";
    return ans;
}
...
int check(
    int i, vector<int> prod, vector<int>& posts,
    pair<int, int> el, vector<int>& dp
) {
