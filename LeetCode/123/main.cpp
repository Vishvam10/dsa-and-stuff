...
class Solution {
  public:
	int maxProfit(vector<int>& arr) {
		...
		// So, we will start from bottom. Base case : (already taken care during
		// init)
		//      - dp[...][...][2] == 0
		//      - dp[n - 1][...][...] = 0
		...
		// max({dp[0][1][0], dp[0][1][1], dp[0][1][2]}) is wrong because it
		// means What if I magically started with 1 or 2 transactions already
		// done ? Which is not allowed by problem definition
		return nxt[1][0];
	}
};
