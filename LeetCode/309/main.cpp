class Solution {
  public:
	int maxProfit(vector<int>& arr) {
		...
		// dp[ind][purchaseState] means at that index, what's the max profit we
		// can get
		...
		// So, we will start from bottom. Base cases : (already taken care
		// during init)
		//      - dp[n][...] = 0
  ...
  public:
	int maxProfit(vector<int>& arr) {
		...
		for (int i = n - 1; i >= 0; --i) {
			...
			// Can buy now
			dp[i][0] = max(-arr[i] + dp[i + 1][2], dp[i + 1][0]);

			// Cooldown only
			dp[i][1] = dp[i + 1][0];

			// Sell only
			dp[i][2] = max(arr[i] + dp[i + 1][1], dp[i + 1][2]);
		}
		return dp[0][0];
	}
};
